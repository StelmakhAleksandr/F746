...
static void multiple_labels_cb(void)
{
    ...
    lv_point_t s;
    lv_text_get_size(&s, "Hello LVGL!", lv_obj_get_style_text_font(lv_screen_active(), 0), 0, 0, LV_COORD_MAX, LV_TEXT_FLAG_NONE);
...
static void summary_create(void)
{
    ...
    for (i = 0; scenes[i].create_cb; i++) {
        ...
        if (scenes[i].measurement_cnt <= 1) {
            ...
            lv_table_set_cell_value_fmt(table, i + 2, 3, "%" LV_PRIu32 " ms (%" LV_PRIu32 " + %" LV_PRIu32 ")", render_time + flush_time, render_time, flush_time);
    ...
    if (valid_scene_cnt < 1) {
        ...
        lv_table_set_cell_value_fmt(table, 1, 3, "%" LV_PRIu32 " ms (%" LV_PRIu32 " + %" LV_PRIu32 ")", render_time + flush_time, render_time, flush_time);
        /* csv log */
